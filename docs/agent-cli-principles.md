...
## Pending principles (#7-#9)

- **#7 Multi-surface architecture.** mk is CLI-only today. An MCP server surface is plausible later but not implemented.
...
When implementing #7 or #8, extend [agent-cli-redesign.md](agent-cli-redesign.md) first with the design pass, then update this doc.
