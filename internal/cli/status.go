...
// statusReport is the unified shape returned by `mk status`. Fields the
// current invocation can't fill (e.g. repo when outside a git repo) are
// emitted as null in JSON.
...
func printStatus(w io.Writer, r *statusReport) error {
	if r.InRepo && r.Repo != nil {
		...
		// Print states in lifecycle order, only those with non-zero counts.
	...
	fmt.Fprintln(w, "Not inside a tracked repo — cd into one, or run `mk init` to register the current one.")
	return nil
}
